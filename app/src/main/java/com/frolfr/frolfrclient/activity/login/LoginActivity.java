...
public class LoginActivity extends Activity implements LoaderCallbacks<Cursor> {
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            Authorization.AuthorizationRequest authRequest = new Authorization.AuthorizationRequest(mEmail, mPassword);

            final String TOKEN = "token";

            String jsonResponse = authRequest.execute();
            if (jsonResponse != null) {
                Log.d(getClass().getName(), "Got a response from ajax UserLoginTask");
                try {
                    return new JSONObject(jsonResponse).getString(TOKEN);
                } catch (JSONException e) {
                    Log.e(getClass().getSimpleName(), "Got a successful authorization response, but no token?", e);
                    return null;
                }
            } else {
                Log.w(getClass().getName(), "No response from ajax UserLoginTask");
                return null;
            }
        }
